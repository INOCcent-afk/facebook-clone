...
export const CoverPhoto: FC<Props> = ({ isEditorMode, coverPhoto }) => {
	const { newCoverPhoto, updateNewCoverPhoto } = useProfileStore();
	...
	const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
		...
		if (e.target.files) {
			if (Array.from(e.target.files).length > MAX_LENGTH) {
				...
				updateNewCoverPhoto(imageFiles[0]);
			}
		}
	};

	const getImage = () => {
		if (isEditorMode && newCoverPhoto) {
			return URL.createObjectURL(newCoverPhoto);
		}
